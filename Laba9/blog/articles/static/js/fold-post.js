...
window.addEventListener("DOMContentLoaded", () => {
  const foldBtns = document.querySelectorAll(".fold-button");
     ...
     foldBtns[i].addEventListener("click", (e) => {
       ...
       e.target.parentElement.getElementsByClassName("article-author")[0].style.display = displayState;
       e.target.parentElement.getElementsByClassName("article-created-date")[0].style.display = displayState;
       e.target.parentElement.getElementsByClassName("article-text")[0].style.display = displayState;
     });
  }
});
*/
